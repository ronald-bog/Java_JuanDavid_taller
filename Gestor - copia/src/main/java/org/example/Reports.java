...
public class Reports {
    public static void generarReporteRiesgos(List<Cliente> clientes, String rutaArchivo) throws IOException {
        ...
        for (Cliente cliente : clientes) {
            ...
            for (Cuenta cuenta : cliente.getCuentas()) {
                ...
                String redTarjeta = "";
                if (cuenta.getRedTarjeta() != null) {
                    switch (cuenta.getRedTarjeta()) {
                        case "V":
                            redTarjeta = "Visa";
                            break;
                        case "MC":
                        ...
                        case "AE":
                            ...
                            break;
                        default:
                            redTarjeta = cuenta.getRedTarjeta(); // Si no es conocido, imprime el valor original
                            break;
                    }
                }

                // Escribir la información de la cuenta
                bw.write(tipoCuenta + " - " + cuenta.getBanco() + " - " + redTarjeta + "\n");
            }

            // Escribir el reporte de comportamiento crediticio
            String reporte = determinarComportamientoCrediticio(cliente.getPuntajeCrediticio());
            bw.write("Reporte: " + cliente.getPuntajeCrediticio() + " - " + reporte + "\n");
            bw.write("#\n");
        }
    ...
    public static void generarArchivoPorBanco(List<Cliente> clientes, String banco, String rutaArchivo) throws IOException {
        ...
        for (Cliente cliente : clientes) {
            for (Cuenta cuenta : cliente.getCuentas()) {
                if (cuenta.getBanco().equals(banco)) {
                    ...
                    // Escribir la información de la cuenta
                    bw.write(tipoCuenta + " " + cuenta.getSaldo() + " " + redTarjeta + "\n");
                    bw.write("#\n");
                    break;
                }
            }
    ...
    public static void obtenerSaldoTotalPorTipoCuenta(List<Cliente> clientes, String idCliente, String tipoCuenta) {
        for (Cliente cliente : clientes) {
            if (cliente.getIdentificacion().equals(idCliente)) {
                ...
                if (cuentaEncontrada) {
                    System.out.println("El saldo total para el cliente " + cliente.getNombre() + " es: " + saldoTotal);
                } else {
